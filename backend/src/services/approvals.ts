...
export const approvalsService = {
  ...
  async getApprovals(filter: ApprovalFilter) {
    return this.getPendingApprovals(filter);
  },

  async getApprovalDetails(receiptId: string, params: any) {
    try {
      const query = `
        SELECT a.*, r.*, u1.first_name as submitter_first_name, u1.last_name as submitter_last_name,
               u2.first_name as approver_first_name, u2.last_name as approver_last_name
        FROM approvals a
        JOIN receipts r ON a.receipt_id = r.id
        JOIN users u1 ON a.submitter_id = u1.id
        LEFT JOIN users u2 ON a.approver_id = u2.id
        WHERE a.receipt_id = $1 AND r.company_id = $2
      `;
      
      const result = await db.query(query, [receiptId, params.companyId]);
      return result.rows[0];
    } catch (error) {
      logger.error('Error getting approval details:', error);
      throw error;
  ...
  async submitForApproval(params: SubmitApprovalParams) {
    try {
      ...
      const query = `
        INSERT INTO approvals (
          id, receipt_id, submitter_id, status, comments, priority,
          requested_approver, created_at
        )
  ...
  async checkApprovalAuthority(params: any) {
    try {
      // Check if user has approval authority for this receipt
      const query = `
        SELECT COUNT(*) as count
        FROM company_users cu
        WHERE cu.user_id = $1 AND cu.company_id = $2 
          AND cu.role IN ('admin', 'approver')
      `;
      
      const result = await db.query(query, [params.approverId, params.companyId]);
  ...
  async processApproval(params: ProcessApprovalParams) {
    try {
      ...
      if (params.tags && params.tags.length > 0) {
        await this.applyTagsToReceipt(params.receiptId, params.tags);
      }
  ...
  async getWorkflowConfig(companyId: string): Promise<WorkflowConfig> {
    try {
      ...
      return {
        ...
        requireApprovalAbove: config.require_approval_above,
        defaultApprovers: JSON.parse(config.default_approvers || '[]'),
        approvalLevels: JSON.parse(config.approval_levels || '[]'),
        notifications: JSON.parse(config.notifications || '{"email": true, "push": true}')
      };
    } catch (error) {
      logger.error('Error getting workflow config:', error);
      throw error;
  ...
  async updateWorkflowConfig(companyId: string, config: Partial<WorkflowConfig>) {
    try {
      const updateFields = [];
      const values = [];
      let paramIndex = 1;

      if (config.autoApprovalThreshold !== undefined) {
        updateFields.push(`auto_approval_threshold = $${paramIndex++}`);
        values.push(config.autoApprovalThreshold);
      }

      if (config.requireApprovalAbove !== undefined) {
        updateFields.push(`require_approval_above = $${paramIndex++}`);
        values.push(config.requireApprovalAbove);
      }

      if (config.defaultApprovers !== undefined) {
        updateFields.push(`default_approvers = $${paramIndex++}`);
        values.push(JSON.stringify(config.defaultApprovers));
      }

      if (config.approvalLevels !== undefined) {
        updateFields.push(`approval_levels = $${paramIndex++}`);
        values.push(JSON.stringify(config.approvalLevels));
      }

      if (config.notifications !== undefined) {
        updateFields.push(`notifications = $${paramIndex++}`);
        values.push(JSON.stringify(config.notifications));
      }
  ...
  async getApprovalStats(params: any) {
    try {
      const query = `
        SELECT 
          COUNT(*) as total,
      ...
      const result = await db.query(query, [params.companyId]);
      return result.rows[0];
    } catch (error) {
      logger.error('Error getting approval stats:', error);
      throw error;
  ...
  async getApprovalHistory(params: any) {
    try {
      ...
      const query = `
        SELECT a.*, r.merchant_name, r.total_amount,
  ...
  async delegateApproval(params: any) {
    try {
      const delegationId = uuidv4();
      
      const query = `
        INSERT INTO approval_delegations (
          id, delegator_id, delegate_to_id, company_id, start_date, end_date,
          max_amount, categories, reason, created_at
        )
